...
test('golden journey: demo, evidence, scenario, compare, profile, roadmap, persistence', async ({
  ...
  await expect(page.getByText('Your saved profile has not changed.')).toBeVisible();
  await expect(page.locator('.causal-diff .diff-stat').first()).not.toHaveText(/^0/);
  await page.locator('.simulation-banner').getByRole('button', { name: 'Apply scenario' }).click();
  ...
  await expect(page.locator('.stats-grid button').first()).toContainText('02');
  await page.getByRole('switch', { name: /Add a valid SAT/ }).check();
  await page.getByRole('button', { name: 'Explore this scenario', exact: true }).click();
